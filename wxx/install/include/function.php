...
function install_sql()
{
	...
	//检测SQL安装文件
	$sql_file = ROOT_PATH.'./install/zd.sql';
...
/*	//采集并安装测试数据
    ...
    return [
        ...
        'dsn' => 'mysql:host=$db_address;post=$db_nmb;dbname=$db_name',
...
/*	$updateData    = array(
	...
	$is_success = create_config($configFile,$updateData);
	if(!$is_success)
	{
...
//根据默认模板生成config文件
function create_config($config_file,$updateData)
{
	...
	return file_put_contents($config_file,$updateData);
}
